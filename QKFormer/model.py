...
class Token_QK_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q_conv_out.unsqueeze(2).reshape(T, B, self.num_heads, C // self.num_heads, N) # Q_{NxD}

        k_conv_out = self.k_conv(x_for_qkv)
        k_conv_out = self.k_bn(k_conv_out).reshape(T, B, C, N)
        k_conv_out = self.k_lif(k_conv_out)
        k = k_conv_out.unsqueeze(2).reshape(T, B, self.num_heads, C // self.num_heads, N) # K_{NxD}
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        # α=exp(−Δt/τ)，τ 记录的是每个时间步的漏电量，τ越小漏电越多，越大累积存电越多
        self.proj_lif = MultiStepLIFNode(tau=2.0, detach_reset=True) 
